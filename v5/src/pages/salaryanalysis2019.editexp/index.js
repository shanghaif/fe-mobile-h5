...
import Jobs4ES from '../../components/business/react-jobs-4ES';
...
const $jobInputVal = $('input', $job).val();
// 是否同步简历
...
// eslint-disable-next-line no-new
if (!$jobInputVal) {
  $job.find('em').html('请选择');
} else {
  const jobVal = Jobs.list[$jobInputVal] ? Jobs.list[$jobInputVal][0] : '';
  $job.find('em').html(jobVal).removeClass('placeholder');
}
const jobs = new Jobs4ES({
  $inputContainer: $jobInput,
  industryCodes: [$jobInput.val()],
  onChange: (code, arr) => {
    const name = arr ? arr[1] : '';
    if (code !== '') {
      $job.find('em').removeClass('placeholder');
    }
    $job.find('em').html(name);
    $jobInput.val(code);
  },
});
$job.on('click', () => {
