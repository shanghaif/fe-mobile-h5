...
$.fn.dataStatistics = function(options){
    options = $.extend({  
      ...
      //执行      
      function run(){
        var difference =options.max-options.min;//要执行动画的次数 max值减最小
        //每次要执行动画的时间
        ...
        function increase() {
          ...
          if( t==1000){
            ...
            if (current.next().next().next().length == 0) {
              ...
              current.next().next().next().addClass('active');
            }
          }
        }
        ...
        //clearInterval(timer1);
        //var timer1 = setInterval(increase,3000);
       }
      //当数字翻到9的时候，前一位数执行一次动画
      function prevNumber(ths,t){
