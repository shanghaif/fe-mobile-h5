...
function isRelease() {
  logger('检测是否Release分支, 检测是否有未提交的更新...', 'blue');
  return new Promise((resolve, reject) => {
    exec('git status -sb', (err, status) => {
      if (err) {
        ...
        const match = status.match(/^## (release-\d{8}-[0-9a-zA-Z.]+)...origin\/\1([^\n]*)\n?\r?([\s\S]*)/);
        if (match) {
          if (match[3]) {
            ...
            logger('Release分支检测成功, 代码变更检测成功.');
            resolve();
          }
        } else {
          reject(new Error('不是Release分支!!!'));
        }
      }
    });
  });
 ...
 */
function isSame() {
  const promises = ['git ls-remote origin -h HEAD|cut -f1', 'git log --pretty=format:"%H"'].map(cmd => new Promise((resolve, reject) => exec(cmd, (err, status) => { err ? reject(err) : resolve(status); })));
  logger('检查release分支是否包含master分支', 'blue');
  return Promise.all(promises)
    .then(([remoteCommitID, trees]) => {
      if (trees && trees.indexOf(remoteCommitID) >= 0) {
        logger('检查release分支是否包含master分支成功.');
      } else {
        return Promise.reject(new Error('当前release分支不是master的分支!!!'));
      }
    });
}
